...
Below are step by step instructions on how to update and upload your app to iTunes Connect using Fastlane. Fastlane is a set of command line tools which automate the app building and deployment process.

## Requirements

In order to use Fastlane, first you need to download Fastlane along with it's dependencies on your computer. You can download Fastlane by doing the following:

1. Opening __Terminal__ and navigating to the app folder with the following command: `cd < PATH TO APP FOLDER >`
2. Enter the following command to download Fastlane: `[sudo] bundle install`

## Automating the with Fastlane

### Setup

Before you can build your iOS app, there is some setup required.
...
_fastlane create_cert_and_prov_ is the same as
